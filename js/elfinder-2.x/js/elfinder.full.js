			...
			attachThumbnails = function(images) {
				...
				$.each(images, function(hash, tmb) {
					...
					if (node.length) {
						(function(node, tmb) {
							$('<img/>')
								.load(function() { node.find('.elfinder-cwd-icon').css('background', "url('"+tmb.replace(/\\/g, '/')+"') center center no-repeat"); })
								.attr('src', tmb);
						})(node, url+tmb);
					} else {
						ret = false;
...
elFinder.prototype.commands.info = function() {
	...
	this.exec = function(hashes) {
		...
		if (tmb) {
			$('<img/>')
				.load(function() { dialog.find('.elfinder-cwd-icon').css('background', 'url("'+tmb.replace(/\\/g, '/')+'") center center no-repeat'); })
				.attr('src', tmb);
		}
			...
			if (file) {
				...
				if (file.tmb) {
					$('<img/>')
						...
						.load(function() {
							icon.css('background', 'url("'+tmb.replace(/\\/g, '/')+'") center center no-repeat');
							$(this).remove();
						})
