...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#addSubButton').button({
            ...
            if (t.selected)
            {
                var element = $(t.selected).find("a");
                var last = element.data("last") == "Y";
                var leaf = element.data("leaf") == "Y";
                if(last && leaf)
                {
                    alert("Выбранный узел не может иметь подкатегорий.")
                    return;
                }
        ...
        $('#tagsButton').button({
            ...
            var id = $.tree.focused().selected.find("a").attr("id").substr(1);
            var link = '?module=<?php echo $moduleName; ?>&method=Tags&top={id}'.replace('{id}', id);
            document.location = link;
        });
        $('#tagsButton').button('disable');
                            ...
                            if (this.settings.rules.multiple != false && this.selected_arr.length > 1) {
                                ...
                                $.each(this.selected_arr, function () {
                                    if (this.find('a').attr("id")) {
                                        val.push(this.find('a').attr("id"));
                                    }
                                });
                                val = val.join(",");
                            }
                            else
                                var val = this.selected.find('a') ? this.selected.find('a').attr("id") : false;
        ...
        $("#datatree_<?php echo $table ?>").tree({
            ...
            callback: {
                ...
                onselect: function (node) {
                    $('#editButton,#delButton,#addSubButton,#listButton,#tagsButton').button('enable');
                },
                ondeselect: function (node) {
                    $('#editButton,#delButton,#addSubButton,#listButton,#tagsButton').button('disable');
                },
...
<div class="controlPanel">
    <button id="addButton" class="btn btn-xs btn-primary">Добавить корневую категорию</button>
    <button id="addSubButton" class="btn btn-xs btn-primary">Добавить подкатегорию</button>
    <button id="editButton" class="btn btn-xs btn-primary">Редактировать свойства</button>
    <button id="listButton" class="btn btn-xs btn-primary">Перейти к товарам</button>
    <button id="delButton" class="btn btn-xs btn-danger">Удалить</button>
    <button id="tagsButton" class="btn btn-xs btn-primary">Характеристики категории</button>
</div>
