...
class MediaFiles
{
    ...
    function ConvertIfNeed ( &$file, &$file_name )
    {
        ...
        if ( empty ( $toType ) || empty ( $fromType ) )
            return false;
    ...
    function AddFile ( $file, $file_name, $module, $module_id = 0 )
    {
        if ( is_file ( $file ) )
        {
            ...
            $max_order = SqlTools::selectRows ( "SELECT MAX(`order`) FROM `prefix_mediafiles` WHERE `module`='$module' AND `module_id`=$module_id" );
    ...
    function DelFile ( $id )
    {
        ...
        $i = array_shift ( SqlTools::selectRows ( "SELECT * FROM `prefix_mediafiles` WHERE `id`=$id", MYSQLI_ASSOC ) );
        ...
        if ( is_file ( $file ) )
        {
            if ( unlink ( $file ) )
            {
                SqlTools::execute ( "DELETE FROM `prefix_mediafiles` WHERE `id`=$id" );
            }
        }
    ...
    function SortFiles ( $sorting )
    {
        ...
        {
            SqlTools::execute ( "UPDATE `prefix_mediafiles` SET `order`='$sort' WHERE `id`=$id" );
        }
        $this->UpdateData ();
    }
    ...
    {
        $medias = SqlTools::selectRows ( "SELECT * FROM `prefix_mediafiles` ORDER BY `order`" );
        $data = array ();
        foreach ( $medias as $i )
        {
            $type = $this->GetType ( $i['filetype'] );
            $i['fileinfo'] = unserialize ( $i['fileinfo'] );
            if ( isset ( $this->icons[$i['filetype']] ) )
            {
                $i['icon'] = '/images/mimetypes/' . $this->icons[$i['filetype']] . '.png';
            }
            else
            {
                $i['icon'] = '/images/mimetypes/empty.png';
            }
