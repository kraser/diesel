 ...
 * @param string $str2
 */
function plural ( $num, $str0, $str1, $str2 )
{
    $n = ( int ) $num;
...
function jabber ( $to, $message )
{
    ...
    $jabber = Starter::app ()->jabber;
    @$conn = new XMPPHP_XMPP (
        $jabber['host'], $jabber['port'], $jabber['user'], $jabber['password'], $jabber['resource'], $jabber['server'], $printlog = false, $loglevel = XMPPHP_Log::LEVEL_INFO
    );
 ...
 * */
function decode64Elf ( $hash )
{
    $lenRoot = 3; //!! Принято, что шифрованный root занимает 3 символа, и его надо отрезать
    // replace HTML safe base64 to normal
    $path = base64_decode ( strtr ( substr ( $hash, $lenRoot ), '-_.', '+/=' ) );
