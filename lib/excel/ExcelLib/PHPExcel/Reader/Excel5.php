...
class PHPExcel_Reader_Excel5 implements PHPExcel_Reader_IReader
{
	...
	public function load($pFilename)
	{
		...
		$docProperties = $this->_phpExcel->getProperties();
		$this->_phpExcel->setProperties($docProperties->setFileName($pFilename));
		...
		while ($this->_pos < $this->_dataSize) {
			...
			switch ($code) {
				...
				case self::XLS_Type_EOF:			$this->_readDefault();			break;
				default:							$this->_readDefault();			break;
			}
		}
		...
		foreach ($this->_sheets as $sheet) {
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
                ...
                foreach ($this->_objs as $n => $obj) {
                    ...
                    if (!$spContainer || $spContainer->getNestingLevel() > 1) {
                        continue;
                    }
...
//			var_dump($bcBitCount);
	 ...
	 * @return string
	 */
	public static function _encodeUTF16($string, $compressed = '')
	{
		if ($compressed) {
